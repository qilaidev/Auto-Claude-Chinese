...
if (fs.existsSync(testRequirementsPath)) {
  ...
  const installResult = spawnSync(
    python.cmd,
    [...python.argsPrefix, '-m', 'pip', 'install', '-r', testRequirementsPath],
    {
      stdio: 'inherit',
      cwd: backendDir,
      windowsHide: true
...
const pytestArgs = [...python.argsPrefix, '-m', 'pytest', '../tests', '-v', '--tb=short'];
const testResult = spawnSync(python.cmd, pytestArgs, {
  stdio: 'inherit',
  cwd: backendDir,
  windowsHide: true
});
